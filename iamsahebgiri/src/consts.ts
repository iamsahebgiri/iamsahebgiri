export const SITE_TITLE = 'Saheb Giri - Software Engineer, Tinkerer, and Developer.';
export const SITE_DESCRIPTION = "Thinking out loud about design, development, and building excellent software.";

export const SEO = {
    "title": SITE_TITLE,
    "author": "Saheb Giri",
    "description": SITE_DESCRIPTION,
    "language": "en-us",
    "siteUrl": "https://iamsahebgiri.vercel.app",
    "siteRepo": "https://github.com/iamsahebgiri/iamsahebgiri",
    "image": "/static/images/avatar.png",
    "socialBanner": "/static/images/twitter-card.png",
    "email": "iamsahebgiri@gmail.com",
    "github": "https://github.com/iamsahebgiri",
    "twitter": "https://twitter.com/iamsahebgiri",
    "facebook": "https://facebook.com/iamsahebgiri",
    "youtube": "https://youtube.com/iamsahebgiri",
    "linkedin": "https://www.linkedin.com/in/iamsahebgiri",
    "locale": "en-US"
}
